...
function getHandler(options, proxy) {
  ...
  if (corsAnywhere.apiHeader) {
    if (!Array.isArray(corsAnywhere.requireHeader)) {
      corsAnywhere.requireHeader = [];
    }

    corsAnywhere.requireHeader.push(corsAnywhere.apiHeader.toLowerCase());
  }
  ...
  var hasKey = function(headers) {
    return !corsAnywhere.apiHeader || !corsAnywhere.apiKey ||
        (Object.hasOwnProperty.call(headers, corsAnywhere.apiHeader) &&
            corsAnywhere.apiKey === headers[corsAnywhere.apiHeader]);
  };
  ...
  return function(req, res) {
    ...
    if (!hasRequiredHeaders(req.headers)) {
      ...
      res.end('Missing required request header.');
      return;
    }

    if (!hasKey(req.headers)) {
      res.writeHead(401, 'Forbidden', cors_headers);
